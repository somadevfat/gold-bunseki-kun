...
/*
 * GetSyncStatus は同期の進捗状況をDBから直接集計して返します。
 */
func (r *SessionRepositoryImpl) GetSyncStatus(ctx context.Context) (*domain.SyncStatus, error) {
	query := `
		SELECT 
			(SELECT COALESCE(MAX(datetime_jst), '1970-01-01'::timestamp) FROM price_candles) as last_candle,
			(SELECT COALESCE(MAX(date), '1970-01-01'::date) FROM session_volatilities) as last_session,
			(SELECT COALESCE(MAX(datetime_jst), '1970-01-01'::timestamp) FROM economic_events WHERE actual IS NOT NULL) as last_event,
	...
	status.LastSessionAt = lastSessionDate
	status.SyncHealth = "Healthy"

	// 24時間以上更新がない場合は Stale 判定
	if time.Since(status.LastCandleAt).Hours() > 24 {
